...
<daily_test>
	...
	<test_item>
		...
		<id></id>
		<category></category>
		<wntd_file_key></wntd_file_key>
	</test_item>
</daily_test>


*/
public class CreatedIntegratedTestSaveAction extends Action
{
	...
	*/
		...
		HttpServletResponse response)
	{
		...
		context.log("CreatedIntegratedTestSaveAction.perform: user_id "+user_id);
		...
		context.log("CreatedIntegratedTestSaveAction.perform: saved_tests size "+saved_tests.size());
		session.setAttribute("saved_tests", saved_tests);
		I18NWebUtility.forceContentTypeAndLocale(request, response, user_opts);
		if (teacher_id != null)
		{
			context.log("CreatedIntegratedTestSaveAction.perform: go to teacher version");
