...
*/
public class CreatedIntegratedTestDisplayAction extends Action
{
	...
	*/
		...
		HttpServletResponse response)
	{
		...
		context.log("CreatedIntegratedTestAction.perform: user_id "+user_id);
		...
		Momento new_m = new Momento("CreatedIntegratedTestDisplayAction", (new Date().toString()), "000", "000");
		ftr.setMomentoObject(user_id, subject, new_m);
		session.setAttribute("test_words", test_words);
		session.setAttribute("test_id", test_id); // so later jsps and actions know what test to work on
